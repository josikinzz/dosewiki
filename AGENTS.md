...
- Social preview cards live at `#/share/<slug>`; each card shows the first route's dosage/duration, uses a molecule SVG when available (fallback to the dose.wiki wordmark), and Hero surfaces the "Open social preview" button to reach it.
