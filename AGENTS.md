...
- `drug_info.smiles` now stores either a single SMILES string or an array of `{ label, smiles }` objects for mixtures (sourced from `src/data/iupacSmilesMap.json`).
- `src/data/titleIupacSmiles.json` lists every article with `{ title, iupac, smiles }` pulled from `articles.json` and `iupacSmilesMap.json`; `smiles` can be a string or a `components` object for mixtures.
- `scripts/generateSmilesMap.mjs` resolves SMILES strings from OPSIN for each article IUPAC and writes the lookup to `src/data/iupacSmilesMap.json`; missing lookups stay `null` in the map so reruns can skip them.
