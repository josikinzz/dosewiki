...
- Social embed shells are generated at `/share/<slug>/` during build (see `scripts/buildSocialSharePages.mjs`); each static page injects OG/Twitter tags with name, categories, first-route dosage/duration, and a molecule SVG fallbacking to the dose.wiki logo before redirecting to the SPA.
