...
const articlesPath = resolve(process.cwd(), 'public/SubstanceIndex.json');
...
async function main() {
  ...
  if (!Array.isArray(articles)) {
    throw new Error('Expected SubstanceIndex.json to contain an array');
  }

  // SubstanceArticle schema: identification.common_name
  const names = articles
    .map((article) => article?.identification?.common_name || article?.title || '')
    .map((name) => name.trim())
