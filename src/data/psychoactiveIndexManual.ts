 ...
 *
 * JSON shape (see `schemas/psychoactive-index-manual.schema.json`):
 * {
...
export interface ManualPsychoactiveIndexConfig {
  version: number;
...
export interface NormalizedManualPsychoactiveIndexConfig {
  version: number;
...
const parseSection = (value: unknown, index: number, parentKey: string): NormalizedManualCategorySection => {
  if (!isObject(value)) {
...
const parseManualConfig = (value: unknown): NormalizedManualPsychoactiveIndexConfig => {
  if (!isObject(value)) {
    throw new Error("Manual psychoactive index config must be an object.");
  }
...
export const psychoactiveIndexManualConfig = parseManualConfig(
  rawManualConfig as unknown as ManualPsychoactiveIndexConfig,
);

export const getManualCategoryByKey = (
  key: string,
): NormalizedManualCategoryDefinition | undefined => {
  return psychoactiveIndexManualConfig.categoryMap.get(slugify(key));
};
